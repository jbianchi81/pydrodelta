...
class Test_RoundDate(TestCase):
    ...
    def test_round_down(self):
        date = self.tz.localize(datetime(2000,1,1,12,0,0,0)) # ZoneInfo('America/Buenos_Aires'))
        timeOffset = timedelta(days=0)
        timeInterval = timedelta(days=1)
        expected_result = self.tz.localize(datetime(2000,1,1,0,0,0,0)) # ZoneInfo('America/Buenos_Aires'))
        result = roundDate(date, timeInterval, timeOffset, "down")
    ...
    def test_round_up(self):
        ...
        timeOffset = timedelta(days=0)
        timeInterval = timedelta(days=1)
        expected_result = self.tz.localize(datetime(2000,1,2,0,0,0,0)) # ZoneInfo('America/Buenos_Aires'))
