...
class Test_Plan_Graph(TestCase):
    ...
    def test_export_graph(self):
        config = yaml.load(open("%s/sample_data/plans/linear_channel_dummy.yml" % os.environ["PYDRODELTA_DIR"]),yaml.CLoader)
        plan = Plan(**config)
        plan.exportGraph(nodes=plan.topology.nodes, output_file="%s/results/linear_channel_dummy.json" % os.environ["PYDRODELTA_DIR"])
        graph = json.load(open("%s/results/linear_channel_dummy.json" % os.environ["PYDRODELTA_DIR"]))
