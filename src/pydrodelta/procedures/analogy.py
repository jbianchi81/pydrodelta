...
class AnalogyProcedureFunction(ProcedureFunction):
    ...
    def run(
        ...
        df_prono_analog = MetodoAnalogia(data,"valor",mes_select,yr_select,self.time_window,self.search_length, self.forecast_length, self.order_by, self.ascending, self.number_of_analogs)
        
        # Mes a formato fecha
        df_prono_analog['month'] = df_prono_analog.apply(lambda x: month2Date(x.year, x.month), axis=1)

        output = df_prono_analog[['month','Prono']].rename(columns={"month":"timestart", "prono":"valor"}).set_index("timestart")        
        
        ## 2 - Calcula errores X serie
        errores = MetodoAnalogia_errores_v2(self.boundaries[0].node_id,data,"valor",self.time_window,self.parameters,outputfile=self._procedure.save_results) # connBBDD=conn
        
        fin_0 = time.time()
        logging.debug('Tiempo : ',round((fin_0-inici_0)/60,2),' minutos')

        return (
            [output], 
            ProcedureFunctionResults(
                data = df_prono_analog,
                parameters = self.parameters,
                statistics = errores.to_dict()
            )
        )
...
def MetodoAnalogia(df,var,mes_obj,yr_obj,vent_resamp,search_length,forecast_length,order_by,ascending,number_of_analogs):
    ...
    _ , dfObj_0, Result_Indic = CalcIndicXFecha(df,yr_obj,mes_obj,search_length)
    ...
    idx_fecha_f = idx_select + forecast_length
    ...
    for index, row in R_Indic_i.iterrows():
        ...
        idx_sim_f = idx_sim + forecast_length
...
def MetodoAnalogia_errores_v2(name_Est,df,var,vent_resamp,ParamMetodo,outputfile : str=None,connBBDD=None) -> DataFrame:
    ...
    for index, row in df_clip.iterrows():
        ...
        df_errorXMes = concat([df_errorXMes,df_union])
