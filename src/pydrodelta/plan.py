...
class Plan(Base):
    ...
    def execute(
        ...
        if self.output_results_file is not None:
            with open("%s/%s" % (os.environ["PYDRODELTA_DIR"], self.output_results_file),"w",encoding='utf-8') as outfile:
                json.dump([p.read_results() for p in self.procedures], outfile, indent=4) 
        if self.output_stats_file is not None:
            with open("%s/%s" % (os.environ["PYDRODELTA_DIR"], self.output_stats_file),"w",encoding='utf-8') as outfile:
        ...
        if self.output_sim_csv:
            ...
            with open("%s/%s" % (os.environ["PYDRODELTA_DIR"], self.output_sim_csv),"w",encoding='utf-8') as outfile:
