...
class Plan(Base):
    ...
    @topology.setter
    def topology(self,value : Union[dict,Topology,str,None]) -> None:
        if isinstance(value, dict):
            ...
            self._topology = Topology(**util.toMapping(yaml.load(f,yaml.CLoader)),plan=self,input_api_config=self.input_api_config,output_api_config=self.output_api_config,s3_config=self.s3_config, base_path=topology_file_path)
            f.close()
        elif value is None:
            self._topology = None
        else:
    ...
    @property
    def save_variable_sim(self) -> Optional[List[SaveVariableSimDict]]:
        """Select simulated variables to save as separate csv"""
        return self._save_variable_sim
    @save_variable_sim.setter
    def save_variable_sim(self, params : Optional[List[SaveVariableSimDict]]) -> None:
        if params is None:
            self._save_variable_sim = None
            return
        self._save_variable_sim : Optional[List[SaveVariableSimDict]] = []
    ...
    def __init__(
            ...
            output_sim_csv: Optional[str] = None,
            qualifiers : Optional[List[str]] = None,
            save_variable_sim : Optional[List[SaveVariableSimDict]] = None,
        ...
        for procedure in self.procedures:
            ...
            for key in proc_dict:
                ...
                try:
                    ...
                    raise Exception("proc_dict['%s'] is not JSON serializable. %s" % (key, str(e)))
            DG.add_node(proc_id,object=proc_dict)
            for b in procedure.boundaries:
